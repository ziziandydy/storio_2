...
>
> ⚠️ **2026-07-16 更新**：後續版本（v1.1.x～v1.16.0）不需重跑 Phase 0~3（App Icons、Bundle ID、隱私宣告等一次性設定已定型）；每次發版實際流程改依 `CLAUDE.md`「iOS 發佈階段」章節（版號更新 → `build:ios` → Xcode Archive → 送審）。最新版本狀態見 `docs/BACKLOG.md`（目前 v1.16.0 build 18 已上架）。
