...
*   **`src/components/`**: 可複用的 UI 元件 (Components)。
    ...
    *   **本機通知 (Local Notifications, v1.14.0)**: `AppOpenReset` (掛於 layout，app 開啟時記錄 engagement、重排程通知、判斷 Primer/Banner 顯示), `NotificationPrimerCard` (新用戶權限請求底部卡片), `NotificationBanner` (舊用戶升級引導 Banner)。
*   **`src/lib/` & `src/utils/`**: 前端工具函式庫與 API client 設定。
    *   **通知核心 (v1.14.0)**: `notification-config.ts` (所有 hardcoded 參數的單一來源，含 JSDoc), `notifications.ts` (`notificationManager`：排程 `reschedule`、行為學習 `recordEngagement`/`getOptimalHour`、智慧忽略 `incrementIgnoredCount`/`resetIgnoredCount`、權限 `checkAndRequestPermission`), `notification-events.ts` (story-added event bus，解耦新增 Storio 與 Banner 顯示)。
...
1.  **使用者探索與搜尋 (Search & Discover)**:
    *   **UI**: 使用者在前端 `/search` 頁面輸入查詢（Auto/AI/Keyword 三模式輪播）；或在 details 頁點擊演員/導演/Studio/類型/作者 chips 直接導向搜尋頁（帶 `pid`/`cid`/`gid`/`author` 精準參數，搜尋框自動帶入該名稱）。
    *   **API**: 呼叫後端 `search.py` 端點（`GET /search/` 標準搜尋、`POST /search/ai` 語意搜尋）。
    *   **Service**: `search_service.py` 處理查詢：
        *   標準搜尋（`search_multi`）：無精準參數時，影視側打 TMDB `search/multi` 並自動偵測人名（`results[0]` 為人物時轉查其完整作品 `discover with_people`，避免被低相關度同名紀錄片誤導），書籍側 free-text 與 `inauthor:` 合併去重；帶精準參數（`pid`/`cid`/`gid`/`author`）時直接查 discover 或 `inauthor:`，跳過偵測，空結果不觸發 AI fallback。
        *   語意搜尋：呼叫 `gemini_service.py`／OpenAI 解析語意後查 TMDB Discover / Google Books。
    *   **Response**: 將標準化的 `Schema` 回傳給前端展示；details response 額外 additive 提供 `cast_refs`/`director_refs`/`genre_refs`/`company_refs`（TMDB numeric ID），供 chips 精準查詢使用，不影響既有欄位。
...
5.  **本機通知 (Local Notifications, v1.14.0)**:
    ...
    *   **行為學習時段**: 開啟 app（weight 1）與記錄 Storio（weight 2）的時段存入 engagement history，≥ 7 筆後 `getOptimalHour()` 計算個人化排程時間，否則用 fallback（21:00 / 20:00）。
    *   **智慧忽略偵測**: per-trigger `ignoredCount`，連續 3 次未回應停止觸發；評分 / 記錄時重置。
    *   **權限引導**: 新用戶記錄首筆後 `NotificationPrimerCard`；舊用戶升級後新增 Storio 觸發 `NotificationBanner`（透過 `notification-events.ts` 解耦）。
    *   **設定入口**: Profile > Notifications（`/profile/notifications`），主開關 + 兩類型開關；主開關 OFF 時細項隱藏。參數全 hardcoded 於 `notification-config.ts`。
