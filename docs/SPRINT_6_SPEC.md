...
*   **詳細規格**:
    ...
    *   **Staggered Alignment (錯位)**: 左右兩欄相對於中間欄需有垂直位移 (Offset)，打破水平線，營造自然的流動感。
    *   **Enrichment (填充策略)**:
        *   **N = 9 (3x3 Grid)**: 這是填滿 9:16 畫面且包含裁切效果的最佳建議值。
        *   **邏輯**: 若當月收藏數小於 9，則重複循環現有海報直到補滿 9 格。
    *   **Gap**: 微距 (4px) 或 無縫。
...
*   **詳細規格**:
    *   **Visual Focus (重心置中)**: 始終保持「作品」位於畫面的垂直核心區域（黃金比例處），書櫃層板隨作品位置調整，避免死板掛於畫面中央。
    *   **Dynamic Shelves (分層門檻)**:
        *   **1 - 7 件**: **單層**置中。
        *   **8 - 14 件**: **雙層**佈局。
        *   **> 14 件**: **三層**佈局。
    *   **Items (Spines)**:
