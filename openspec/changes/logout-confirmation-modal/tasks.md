...
- [ ] 1.1 修改 `client/src/i18n/locales.ts`：在 `common` 區塊新增 `logout_confirm_title`（確定要登出嗎？ / Sign out?）與 `logout_confirm_cta`（登出 / Sign Out）

## 2. Profile 頁面修改

- [ ] 2.1 在 `client/src/app/profile/page.tsx` 新增 `showLogoutModal` state（`useState(false)`）
- [ ] 2.2 將登出按鈕 `onClick` 從 `handleSignOut` 改為 `() => setShowLogoutModal(true)`
- [ ] 2.3 在 return JSX 末端加入 `<AnimatePresence>` 包裹的 logout 確認 modal：
...
- [ ] 3.1 點擊登出 → modal 以動畫進場，不立即登出
- [ ] 3.2 點擊取消 → modal 關閉，維持登入狀態
- [ ] 3.3 點擊背景遮罩 → 同取消
- [ ] 3.4 點擊「登出」→ 執行登出，導向首頁
- [ ] 3.5 切換 App 語言後重新開 modal → 文字正確切換（zh-TW / en-US）
