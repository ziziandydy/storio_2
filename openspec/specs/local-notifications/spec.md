...
## Purpose
TBD - created by archiving change local-notifications. Update Purpose after archive.
## Requirements
### Requirement: Notification Config

系統 SHALL 以 `client/src/lib/notification-config.ts` 為唯一的 hardcoded 參數來源。所有通知邏輯均從此讀取常數，禁止在其他檔案散落魔法數字。
...
系統 SHALL 在距上次記錄超過間隔天數時，排程個人化的「Log a story」提醒通知。
...
系統 SHALL 在有未評分收藏或心得逾期時，排程個人化的「Folio reflection」提醒通知。
...
系統 SHALL 為每個觸發類型在 localStorage 各自維護 `ignoredCount` 與 `lastSentAt`，並在連續忽略達閾值時停止該觸發路徑。
...
系統 SHALL 限制每日通知不超過 `MAX_PER_DAY`，且 SHALL 將落在 blackout 時段（00:00–08:00）的排程時間推延至 08:00。
...
系統 SHALL 在使用者記錄至少一筆 Storio 後，於下次開啟 app 時顯示 Permission Primer 底部卡片以請求通知權限。
...
系統 SHALL 對升級後未看過 Primer 的既有用戶，於新增 Storio 成功後顯示非阻斷式 Banner 引導開啟通知。
...
系統 SHALL 在 iOS 通知權限被拒時，提供引導 Toast 與前往設定的入口，並維持主開關為關閉狀態。
